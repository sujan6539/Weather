...
abstract class BaseDataBoundAdapter<T : ViewDataBinding> : RecyclerView.Adapter<DataBoundViewHolder<T>>() {
    ...
    /**
     * Scroll the view to given position.
     */
    protected open fun scrollViewToPosition(position: Int) {
        if (mRecyclerView != null && mRecyclerView!!.adapter != null && position > 0
            && position < mRecyclerView!!.adapter!!.itemCount) {
